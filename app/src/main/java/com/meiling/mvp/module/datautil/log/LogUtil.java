...
public class LogUtil {
    ...
    /**
     * 能够进行动态申请权限的日志记录（允许进行日志记录）
     */
    public final void e(Activity context, String msg) {
        if (LOG_E >= LOG_LEVEL && msg != null && msg.length() > 0) {
            Log.e(TAG, msg);
        }
        ...
        if (BuildConfig.LOG_ERROR_SAVE_SWITCH) {
            //TODO 调用saveErrorIntoFile方法保存需要记录的错误日志
            saveErrorIntoFile(context, msg);
        }
    }

    public final void e(AppCompatActivity context, String msg) {
        if (LOG_E >= LOG_LEVEL && msg != null && msg.length() > 0) {
            Log.e(TAG, msg);
        }
    ...
    public final void e(android.app.Fragment context, String msg) {
        if (LOG_E >= LOG_LEVEL && msg != null && msg.length() > 0) {
            Log.e(TAG, msg);
        }
    ...
    public final void e(android.support.v4.app.Fragment context, String msg) {
        if (LOG_E >= LOG_LEVEL && msg != null && msg.length() > 0) {
            Log.e(TAG, msg);
        }
    ...
    public void saveErrorIntoFile(final String errorMsg) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            ...
            recordMsg(errorMsg);//TODO 针对 23 及以下的做文件记录，由于无法动态申请权限，所以在6.0（23）以及以上的系统可能存在问题
        }
    }

    public void saveErrorIntoFile(final Activity activity, final String errorMsg) {
        /**
         * TODO 先检查文件是否存满----每个文件限制大小为2M
         *
         * 文件命名格式为 yyyy-MM-dd_imei（IMEI其实只是用于区分设备，若实际满足不了需求，建议修改命名方式，例如：用户编号等
        ...
        //TODO 先校验外存是否处于挂载状态，避免由于外存状态问题导致的读写异常
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && LogPermissionCheck.checkPermission(activity)[0]) {
                recordMsg(errorMsg);
            }
        }
    }

    public void saveErrorIntoFile(final AppCompatActivity activity, final String errorMsg) {
        /**
         * TODO 先检查文件是否存满----每个文件限制大小为2M
         *
         * 文件命名格式为 yyyy-MM-dd_imei（IMEI其实只是用于区分设备，若实际满足不了需求，建议修改命名方式，例如：用户编号等
    ...
    public void saveErrorIntoFile(final android.app.Fragment activity, final String errorMsg) {
        /**
         * TODO 先检查文件是否存满----每个文件限制大小为2M
         *
         * 文件命名格式为 yyyy-MM-dd_imei（IMEI其实只是用于区分设备，若实际满足不了需求，建议修改命名方式，例如：用户编号等
    ...
    public void saveErrorIntoFile(final android.support.v4.app.Fragment activity, final String errorMsg) {
        /**
         * TODO 先检查文件是否存满----每个文件限制大小为2M
         *
         * 文件命名格式为 yyyy-MM-dd_imei（IMEI其实只是用于区分设备，若实际满足不了需求，建议修改命名方式，例如：用户编号等
    ...
    private void recordMsg(final String errorMsg) {
        synchronized (errorQueue) {//TODO 由于需要保证队列中数据的写入与取出的原子性，将该
            errorQueue.add(errorMsg);//TODO 将消息加入队列-----由于
        }
    }

    private void recordMsg(Context context,final String errorMsg) {
        synchronized (errorQueue) {//TODO 由于需要保证队列中数据的写入与取出的原子性，将该
            errorQueue.add(errorMsg);//TODO 将消息加入队列-----由于
